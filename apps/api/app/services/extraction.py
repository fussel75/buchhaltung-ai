...
def _build_pdf_text_result(document: dict) -> dict:
    ...
    project = _resolve_project_for_delivery_addresses(delivery_addresses) if delivery_addresses else find_project_by_address(delivery_address)
    project_code = project.code if project else None
...
def _resolve_project_for_delivery_addresses(delivery_addresses: list[str]):
    ...
    if len(delivery_addresses) == 1 and projects[0]:
        return projects[0]
    if len(project_codes) == 1 and len(projects) == len([project for project in projects if project]):
        return next(project for project in projects if project)
    return None
